# Experimental FBT Unreal Kit Installer
...
4. Keep this complete `FBTUnrealKit` package together. The installer expects its parent folder to contain `FBTUnrealKit.uplugin` and `Source`.

5. Double-click `Install-FBTUnrealKit.bat`. Windows may show a warning because the script is not a signed commercial installer.
...
11. Open the project and confirm `FBT Unreal Kit` is enabled. Run tracker, calibration, and Blueprint-node tests in a test map before using the plugin on valuable work.
...
```powershell
.\Install-FBTUnrealKit.ps1 "D:\Projects\MyGame\MyGame.uproject" -PlanOnly -NonInteractive -InstallerPassword "VirtualMocap-Experimental-2026"
```
