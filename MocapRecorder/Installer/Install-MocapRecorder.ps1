...
function Write-InstallerStatus {
    ...
    Write-Host "[MocapRecorder Installer] $Message"
}

function Select-UnrealProject {
    Add-Type -AssemblyName System.Windows.Forms
    $dialog = New-Object System.Windows.Forms.OpenFileDialog
    $dialog.Title = 'Select the Unreal project that should receive Mocap Recorder'
...
function Enable-PluginInDescriptor {
    ...
    $entry = $plugins | Where-Object { $_.Name -eq 'MocapRecorder' } | Select-Object -First 1
    if ($null -eq $entry) {
...
$pluginDescriptorPath = Join-Path $pluginRoot 'MocapRecorder.uplugin'
...
$hasMatchingBinaries =
    ...
    (Test-Path -LiteralPath (Join-Path $pluginRoot 'Binaries\Win64\UnrealEditor-MocapRecorder.dll'))
...
$plan = [pscustomobject]@{
    ...
    Destination = (Join-Path $projectDirectory 'Plugins\MocapRecorder')
}
...
if ($shouldBuild -and -not $buildToolAvailable) {
    throw "Unreal Engine '$engineAssociation' is missing UnrealBuildTool. Verify this engine installation in Epic Games Launcher, or install a matching precompiled Mocap Recorder package."
}
...
$backupRoot = Join-Path $projectDirectory "Saved\MocapRecorderInstallerBackups\$timestamp"
$destinationPlugin = Join-Path $projectDirectory 'Plugins\MocapRecorder'
...
if (Test-Path -LiteralPath $destinationPlugin) {
    ...
    if (-not $samePluginLocation) {
        Copy-Item -LiteralPath $destinationPlugin -Destination (Join-Path $backupRoot 'MocapRecorder') -Recurse -Force
...
if (-not $samePluginLocation) {
    ...
    $backupPresetRoot = Join-Path $backupRoot 'MocapRecorder\Source\Presets'
...
$logRoot = Join-Path $projectDirectory 'Saved\MocapRecorderInstaller'
...
if (-not $NonInteractive) {
    [System.Windows.Forms.MessageBox]::Show(
        "Mocap Recorder was installed successfully.`n`nBackup: $backupRoot",
        'Mocap Recorder Installed',
