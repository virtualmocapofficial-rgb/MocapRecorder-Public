...
namespace
{  
FString BuildStandaloneBakeAssetName(const UObject* SourceObject, const FString& OverrideName)
{
...
bool UMocapRecorderComponent::ShouldForceAnimationEvaluation() const
{
    ...
    if (bRespectPhysicsDrivenPose && TargetSkeletalMesh->IsAnySimulatingPhysics())
    {
        return false;
    }
...
{
    if (bAutoStopRecordingOnPIEEnd &&
        bIsRecording &&
        EndPlayReason == EEndPlayReason::EndPlayInEditor)
    {
        UE_LOG(LogMocapRecorder, Warning,
            TEXT("MocapRecorder: PIE ended while recording; stopping and queueing the final bake snapshot."));
        StopRecording(false);
    }
...
UMocapRecorderComponent* UMocapRecorderComponent::CreateBakeSnapshot() const
{
    ...
    Snapshot->RecordedCameraTracks = RecordedCameraTracks;

    Snapshot->Frames = Frames;
    Snapshot->TransformFrames = TransformFrames;
...
void UMocapRecorderComponent::StartRecording(bool bInTransformOnly)
{
    ...
    if (TargetSkeletalMesh)
    {
        const int32 NumMaterials = TargetSkeletalMesh->GetNumMaterials();
        RecordedMaterialOverrides.Reserve(NumMaterials);
        for (int32 MaterialIndex = 0; MaterialIndex < NumMaterials; ++MaterialIndex)
        {
            RecordedMaterialOverrides.Add(TargetSkeletalMesh->GetMaterial(MaterialIndex));
        }
    }
...
#if WITH_EDITOR
    if (bAutoQueueBakeOnPIEEnd && NumFrames > 0)
    {
        if (UMocapRecorderComponent* Snapshot = CreateBakeSnapshot())
        {
            GMocapRecordingStoppedForBake.Broadcast(
                Snapshot,
                BuildStandaloneBakeAssetName(this, AutoBakeAssetNameOverride),
...
bool UMocapRecorderComponent::CaptureCurrentPoseToFrame(FMocapFrame& OutFrame)
{
    ...
    // For regular animated characters we force evaluation so the recorder sees the latest graph output.
    // For ragdolls / physics-driven meshes we must not do that, or we fight the simulated pose every sample.
    if (ShouldForceAnimationEvaluation())
    {
...
// - bPreserveStartingLocation=false => baseline = actor root at frame 0 (rebase-to-actor-start)
    if (!bHasWorldBakeBaseline)
    {
        ...
        // Preserve actor/component scale in the baked result instead of normalizing it away.
        WorldBakeBaselineRoot.SetScale3D(FVector::OneVector);
    ...
    OutFrame.Scales.SetNum(NumExportBones);
    ...
    for (int32 BoneIdx = 0; BoneIdx < NumExportBones; ++BoneIdx)
    {
        ...
        OutFrame.Scales[BoneIdx] = Local.GetScale3D();
...
void UMocapRecorderComponent::SampleFrame()
{
    ...
    if (CaptureMode == EMocapCaptureMode::TransformOnly)
    {
        ...
        const float Dt = (SampleRate > 0.f) ? (1.f / SampleRate) : (1.f / 60.f);
        SampleAttachedCameras(TimeFromStart);
    ...
    Frame.Scales = MoveTemp(Captured.Scales);
...
void UMocapRecorderComponent::SampleAttachedCameras(float SampleTime)
{
    if (!bRecordAttachedCameras)
    {
        return;
    }
    ...
    for (UCameraComponent* CameraComponent : CameraComponents)
    {
        ...
        FMocapRecordedCameraTrack* Track = RecordedCameraTracks.FindByPredicate(
            [CameraComponent](const FMocapRecordedCameraTrack& Candidate)
            {
                return Candidate.ComponentName == CameraComponent->GetFName();
            });
        if (!Track)
        {
            Track = &RecordedCameraTracks.AddDefaulted_GetRef();
            Track->ComponentName = CameraComponent->GetFName();
        }

        FMocapCameraFrame& Frame = Track->Frames.AddDefaulted_GetRef();
        Frame.Time = SampleTime;
        Frame.RelativeTransform = CameraComponent->GetComponentTransform().GetRelativeTransform(Owner->GetActorTransform());
        Frame.FieldOfView = CameraComponent->FieldOfView;
        Frame.AspectRatio = CameraComponent->AspectRatio;
        Frame.OrthoWidth = CameraComponent->OrthoWidth;
        Frame.bOrthographic = CameraComponent->ProjectionMode == ECameraProjectionMode::Orthographic;
    }
}
...
void UMocapRecorderComponent::ClearRecordedData()
{
    Frames.Reset();
    TransformFrames.Reset();
    RecordedCameraTracks.Reset();
...
int32 UMocapRecorderComponent::GetRecordedFrameCount() const
{
    return (CaptureMode == EMocapCaptureMode::TransformOnly)
        ? TransformFrames.Num()
        : Frames.Num();
}
